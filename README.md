...
> **關於 `alpine:latest` 的選擇**  
> 在本專案的 `grafana-setup` 服務中，我們刻意選用了 `alpine:latest` 作為基礎映像檔。
> *   **原因**：此容器僅用於執行一次性的初始化腳本 (Shell Script)，依賴極少 (`curl`, `jq`) 且不涉及複雜的系統相容性。
> *   **優勢**：確保總是使用最新的安全性修補版本，且在 Demo 環境中能最大程度減少手動維護舊映像檔的心智負擔。使用 `latest` 在此情境下是权衡後的最佳選擇。
